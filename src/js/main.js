...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const dataSelectors = document.getElementsByName('dataSelector');

  dataSelectors.forEach(node => {
    node.addEventListener('click', (event) => {
      dataService.pickDataset(event.target.value);
      updateCharts();
    });
  });
  ...
  const slider = new sliderChart;
  const usFlight = new flightChart;
  const thirdChart = new barChart;

  // Wait till all data loads

  dataService.getInitialData().then(res => {
    if (res) {
      spinnerContainer.remove();
      // Invoking charts
      usFlight.data(dataService.getMapFlightsData());
      slider.data(dataService.getSliderData());
      thirdChart.data(dataService.getThirdChartData());
      d3.select('.thirdChart').call(thirdChart);
      d3.select('.sliderChart').call(slider);
      d3.select('.flightChart').call(usFlight);
      usFlight.stateFilter(filterDataByState);
      slider.filter(filterData);
    } else {
      spinnerContainer.querySelector('.spinner-text').innerHTML = 'Error happened!';
      spinnerContainer.querySelector('.spinner').remove();
    }
  });

  const showFlightsCheckbox = document.getElementById('showFlights');
  showFlightsCheckbox.addEventListener('change', (event) => {
    usFlight.toggleFlights(event.target.checked);
    throttlePage();
  });

  function updateCharts() {
    const flights = dataService.getMapFlightsData();
    const dates = dataService.getSliderData();
    const thirdChartValues = dataService.getThirdChartData();
    usFlight.data(flights);
    slider.data(dates);
    thirdChart.data(thirdChartValues);
    throttlePage();
  }

  function filterData(isMin, filteringDate) {
    isMin ? dataService.min = filteringDate : dataService.max = filteringDate;
    const flights = dataService.getMapFlightsData();
    const thirdChartValues = dataService.getThirdChartData();
    usFlight.data(flights);
    thirdChart.data(thirdChartValues);
  ...
  function filterDataByState(stateId) {
    dataService.toggleState = stateId;
    const flights = dataService.getMapFlightsData();
    const thirdChartValues = dataService.getThirdChartData();
    usFlight.data(flights);
    thirdChart.data(thirdChartValues);
  ...
  function throttlePage() {
    ...
    setTimeout(() => {
      document.body.classList.remove('not-available');
    }, 1600);
  }
